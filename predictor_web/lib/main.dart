...
class _DashboardScreenState extends State<DashboardScreen> {
        ...
        child: Form(
          ...
          child: Column(
            children: [
              ...
              _buildNumberField(customerController, '客数'),
              _buildNumberField(staffCountController, 'スタッフ数'),
              _buildDateField("シフト開始日時", _shiftStart, (picked) => setState(() => _shiftStart = picked)),
              _buildDateField("シフト終了日時", _shiftEnd, (picked) => setState(() => _shiftEnd = picked)),
              _buildStaffMultiSelect(),
              DropdownButtonFormField<String>(
                value: festivalStatus,
                decoration: const InputDecoration(labelText: '祭りの有無'),
  ...
  Widget _buildDateField(String label, DateTime? date, Function(DateTime) onPicked) {
    return ListTile(
      title: Text(label),
      subtitle: Text(
        date == null ? '日付を選択' : '${date.year}/${date.month}/${date.day}',
      ),
      trailing: IconButton(
        icon: const Icon(Icons.calendar_today),
        onPressed: () async {
